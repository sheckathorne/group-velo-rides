...
# @shared_task
# def fetch_weather_data(location, zipcode=None, forecast_date=None, force_refresh=False):
#     """
...
#     """
#     try:
#         # Convert forecast_date to date object if it's a string
#         if forecast_date and isinstance(forecast_date, str):
#             forecast_date = timezone.datetime.strptime(forecast_date, "%Y-%m-%d").date()
#         else:
...
#                 location=location, zipcode=zipcode, forecast_date=forecast_date
#             )
#             if cached_data:
#                 print(f"Using cached weather data for {location} on {forecast_date}")
#                 return cached_data.to_dict()

#         # If we're here, we need fresh data
#         print(f"Fetching fresh weather data for {location} on {forecast_date}")
#         weather_service = WeatherService()
#         weather_data = weather_service.get_weather_data(location, zipcode, forecast_date)
...
#         if zipcode:
#             existing_record = existing_record.filter(zipcode=zipcode)
#         elif location:
